...
static int uECC_sign_with_k_internal(const uint8_t *private_key,
    ...
    uECC_word_t py[uECC_MAX_WORDS];// y coord
    memcpy(py, p + 8, sizeof(py));
    uint32_t parity = py[0] & 1;
    if(parity)
        printf("EVEN\n");
    else
        printf("ODD\n");
...
int uECC_sign_with_k(const uint8_t *private_key,
    ...
    return uECC_sign_with_k_internal(private_key, message_hash, hash_size, k2, signature, curve, NULL);
}
...
int uECC_sign(const uint8_t *private_key,
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        if (uECC_sign_with_k_internal(private_key, message_hash, hash_size, k, signature, curve, NULL)) {
            return 1;
        }
    }
    return 0;
...
int uECC_sign_deterministic(const uint8_t *private_key,
    ...
    filled:
        ...
        if (uECC_sign_with_k_internal(private_key, message_hash, hash_size, T, signature, curve, pby)) {
            return 1;
        }
